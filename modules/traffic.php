...
if(isset($_GET['getinfo']) && preg_match("/^[0-9]{1,10}$/", $_GET['getinfo'])) {
	...
	if(is_array($rs)) {
		...
		if(isset($hs['status']) && $hs['status'] == 200) {
			...
			$tf_in  = chhcn\Utils::getFormatTraffic($js['total_traffic_in']);
			$tf_out = chhcn\Utils::getFormatTraffic($js['total_traffic_out']);
			echo <<<EOF
	...
	if(is_array($rs)) {
		...
		if(isset($hs['status']) && $hs['status'] == 200) {
			...
			foreach($js['proxies'] as $proxy) {
				...
				$tf_in  = chhcn\Utils::getFormatTraffic($proxy['today_traffic_in']);
				$tf_out = chhcn\Utils::getFormatTraffic($proxy['today_traffic_out']);
